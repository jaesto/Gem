...
function ensureLayoutRegistered() {
  ...
  if (typeof cytoscapeCoseBilkent !== 'undefined') {
    plugin = cytoscapeCoseBilkent;
  } else if (typeof window !== 'undefined' && typeof window.cytoscapeCoseBilkent !== 'undefined') {
    plugin = window.cytoscapeCoseBilkent;
  }
  if (typeof plugin === 'function') {
    try {
      ...
      if (!/already registered/i.test(error?.message || '')) {
        console.warn('Failed to register cose-bilkent layout', error);
      }
    }
    state.layoutRegistered = true;
  }
}

function runAutoLayout(options = {}, onStop) {
  if (!state.cy) return null;
  const layoutOptions = { ...REFRESH_LAYOUT_OPTIONS, ...options };
  state.cy.stop();
